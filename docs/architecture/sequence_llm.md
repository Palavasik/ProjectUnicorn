...
1. **`log_job_completed`** — сводка: пользователь, стартовая точка, **запрошенная дистанция** (км, выбор на шаге дистанции), нумерованный список маршрутов, длительность сессии.
2. **`log_llm_response`** — в одном сообщении: **время ответа LLM** (секунды, замер вокруг `get_routes_from_llm`), **текст запроса** к API (промпт после подстановки `{{lat}}`, `{{lon}}`, `{{distance_km}}`), имя **модели** (`OPENROUTER_MODEL`) и **полный** `message.content` ответа (до разбора JSON). Оба длинных блока в `<pre>`, при необходимости обрезаются с пометкой «обрезано». Экранирование через `html.escape`.
