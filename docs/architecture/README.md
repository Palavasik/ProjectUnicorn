...
```
/find → [Точка старта: геолокация/координаты] → [Выбор дистанции: 3 кнопки] → вызов OpenRouter с промптом из проекта → список маршрутов (название, описание) + под каждым кнопка «Построить маршрут в Яндекс.Картах» → по нажатию: ссылка на Яндекс.Карты с координатами маршрута
```
