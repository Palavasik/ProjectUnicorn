...
```
/find → [Точка старта: геолокация / координаты / адрес (геокодер)] → [Выбор дистанции: 3 кнопки] → вызов OpenRouter с промптом из проекта → список маршрутов (название, описание) + под каждым кнопка «Построить маршрут в Яндекс.Картах» → по нажатию: ссылка на Яндекс.Карты с координатами маршрута
```
...
**Путь пользователя в интерфейсе (CJM):** **[../bot/CJM.md](../bot/CJM.md)** — шаги, тексты, форматы ввода/вывода, кнопки.

Подробная последовательность вызова LLM (параметры, промпт, формат ответа, Яндекс.Карты): **[sequence_llm.md](sequence_llm.md)**.

## Технологический стек MVP

- **Язык**: Python 3.10+
- **Фреймворк**: python-telegram-bot 20.7
- **Поиск маршрутов**: OpenRouter (Chat Completions), промпт из файла проекта
...
- **geocode_service.py** — адрес → координаты (Яндекс при `YANDEX_GEOCODER_API_KEY`, иначе Nominatim)
- **llm_route_service.py** — вызов OpenRouter с промптом из файла (подстановка lat, lon, distance_km), парсинг JSON-ответа с маршрутами (name, description, coordinates)
